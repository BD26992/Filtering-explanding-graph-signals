...
T_b=250;T_e=255;hypsig=1;
D=squareform(pdist(Position(existing,:),'euclidean'));N_links=10;type='directed';
N=size(D,1);
D_bar=sum(D(:))/(N*(N-1));
D=exp(-D/D_bar);
D=NN(D,N_links,type);
...
[A_Trn,a_Trn,x_Trn,A_Tst,a_Tst,x_Tst] = build_pos(R_e,R_i,Position,existing,incoming,N_start,N_incoming,T_b,T_e+H,A_0,x_0,frac,N_links,C,hypsig,sim_type);
...
for r=1:200
[~,rnmse_seq_ada(r),~,~,~,~,~,p_t{i,j},w_t{i,j},~,~] = online_adaptive(A_Trn,x_Trn,a_Trn,L,step_p(1),mu_p(1),N_links,h_i,P,W,eta_p(i),eta_w(j),scale);
end
