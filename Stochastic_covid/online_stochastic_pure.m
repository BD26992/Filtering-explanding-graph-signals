function [squared_error2,rnmse_seq,h,A_latest,x_latest,a_latest,H] = online_stochastic_pure(A_T,x_T,a_T,L,step,mu,nlinks,h_i)
...
squared_error2(i)=((w.*p)'*A_x*h-x_true(i))^2+(A_x*h)'*diag(w.*w.*p.*(ones(N,1)-p))*(A_x*h)+mu*norm(h)^2;
%% Update filter
