...
function [squared_error2,rnmse_seq,h,A_latest,x_latest,a_latest,y_latest,H] = online_proposed(A_T,x_T,a_T,y_T,L,step,mu,h_i)
%% Description
...
%% Error
   ...
   %grad(:,l)=((x_pred-xx(j))*A_x'*att+2*mu*h);
   %h=h-(step/(norm(grad,'fro')))*grad(:,l);
%% Update filter
