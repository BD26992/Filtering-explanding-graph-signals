function [squared_error2,rnmse_seq,h,A_latest,x_latest,a_latest,y_latest,H] = online_stochastic(A_start,x_T,y_T,a_T,L,step,mu,nlinks,h_i)
%% Description
...
%% Error
   ...
   squared_error2(l)=squared_error(i)+mu*norm(h)^2;
%% Update filter
   h=h-step*((A_x'*att*att'*A_x+mu)*2*h-A_x'*att*xx(j));
%   if norm(h)>1
%        h=h/norm(h);
%   end
