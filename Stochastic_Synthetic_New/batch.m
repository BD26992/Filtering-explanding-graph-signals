function [se_sequence,rnmse_sequence,h_batch] = batch(A_T,x_T,a_T,L,gamma_batch)
%% Inputs
...
h_batch=pinv(A'*A+T*gamma_batch*eye(L+1))*A'*x;% least square batch solution
se_sequence=(((A*h_batch-x).*(A*h_batch-x))');
