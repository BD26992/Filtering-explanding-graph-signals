...
function [squared_error_2,rnmse_seq,h,A_latest,x_latest,a_latest,H] = online_proposed(A_T,x_T,a_T,L,step,mu,h_i,scale)
%% Inputs
...
x_pred=scale*att'*A_x*h;
%% Error
   squared_error(i)=(x_pred-x_true(i))^2;
   squared_error_2(i)=(x_pred-x_true(i))^2+mu*norm(h)^2;
%% Update filter
   h=h-(step)*(scale*(x_pred-x_true(i))*[A_x'*att]+2*mu*h);
% if norm(h)>=10000
% h=10000*h/(norm(h));
% end
