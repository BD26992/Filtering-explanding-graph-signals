function [squared_error2,rnmse_seq,h,A_latest,x_latest,a_latest,H,p_bar,D] = online_adaptive2(A_T,x_T,a_T,D_T,L,step,mu,nlinks,h_i,P,eta_p,scale)
%% Description
...
c=A_x*h;
grad_p=scale*(x_pred(i)-x_true(i))*D'*(w.*c)+scale^2*(D'*(c.*c.*w.*w)-2*D'*(p.*c.*c.*w.*w));
